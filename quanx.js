...
function CT2QX(cnt) {
  ...
  ipt = cnt.server+":"+cnt.port
  pwd = "password=" + cnt.password
  var sni = cnt.sni || cnt.servername || cnt["server-name"] || ""
  otls = "over-tls=true"
  opath=""
  ohost = sni ? "tls-host="+sni : ""
  ...
  tls13 = PTls13 == 1 ? "tls13=true" : "tls13=false"
  var udpEnabled = Pudp0 == 1 ? true : Pudp0 == -1 ? false : !!cnt.udp
  udp = udpEnabled ? "udp-relay=true" : "udp-relay=false"
  tfo = cnt.tfo ? "fast-open=true" : "fast-open=false"
  if (cnt.network=="ws") { //wss类型
    otls = "obfs=wss"
    //$notify("trojan","WS",JSON.stringify(cnt))
    if (cnt["ws-opts"]){
      var wsOpts = cnt["ws-opts"]
      var wsHeaders = wsOpts["headers"] || {}
      var wsHost = wsHeaders["Host"] || wsHeaders["host"] || sni
      opath = wsOpts["path"] ? "obfs-uri="+wsOpts["path"] : ""
      ohost = wsHost ? "obfs-host="+wsHost : ""
    } else {
      ohost = sni ? "obfs-host="+sni : ""
    }
