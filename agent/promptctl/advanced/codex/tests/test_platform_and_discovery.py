...
def test_multi_directory_successful_deployment(tmp_path, monkeypatch):
    ...
    for directory in directories:
        assert (directory / "your-rules.md").read_text(encoding="utf-8") == (
            codex_instruct.BUILTIN_GPT_UNRESTRICTED_MD
        )
        assert 'model_instructions_file = "./your-rules.md"' in (
...
def test_atomic_no_replace_has_one_winner_across_processes(tmp_path):
    ...
    outputs = []
    try:
        server_context = socket.create_server(("127.0.0.1", 0))
    except PermissionError as exc:
        pytest.skip(f"TCP socket binding is blocked by the test sandbox: {exc}")
