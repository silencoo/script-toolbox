...
def test_explicit_english_empty_modes_are_fully_localized(tmp_path):
    ...
    for arguments, returncode, expected in cases:
        deployment_arguments = arguments
        if "--dry-run" in arguments:
            deployment_arguments = (*arguments, "--file", str(EXAMPLE_PATH))
...
def test_dry_run_discloses_prompt_source_hash_and_global_behavior(tmp_path):
    codex_dir = _make_codex_dir(tmp_path)
    expected_hash = hashlib.sha256(EXAMPLE_PATH.read_bytes()).hexdigest()
    ...
    assert "[Prompt] Source: external file" in english.stdout
    assert str(EXAMPLE_PATH) in english.stdout
    ...
    assert "[提示词] 来源: 外部文件" in chinese.stdout
    assert str(EXAMPLE_PATH) in chinese.stdout
...
def test_dry_run_discloses_external_prompt_source_and_hash(tmp_path):
    ...
    assert "[Behavior notice]" in result.stdout
...
def test_frozen_schema1_manifest_supports_external_rewrite_field_only_restore(
    ...
    external_rewrite = (
        ...
        'model_instructions_file = "./your-rules.md" # external rewrite\n'
    )
...
def test_ccswitch_rewrite_preserves_unrelated_config_and_restores_old_reference(
    ...
    rewritten = (
        ...
        'approval_policy = "never"\n'
        'model_instructions_file = "./your-rules.md" # kept by CCSwitch\n'
    )
...
def test_ccswitch_rewrite_of_unchanged_owned_reference_is_not_touched(tmp_path):
    codex_dir = _make_codex_dir(
        tmp_path,
        config='model_instructions_file = "./your-rules.md"\n',
    )
    _deploy(codex_dir)
    manifest = json.loads(
...
def test_uninstall_uses_portable_ownership_not_inode_only(tmp_path):
    codex_dir = _make_codex_dir(tmp_path)
    _deploy(codex_dir)
    md_path = codex_dir / "your-rules.md"
