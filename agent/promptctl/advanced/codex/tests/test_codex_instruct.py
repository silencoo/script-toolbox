...
EXAMPLE_PATH = MODULE_PATH.parent / "examples" / "your-rules.md"
...
def test_normalize_md_name_accepts_simple_names():
    assert codex_instruct.normalize_md_name("your-rules") == "your-rules.md"
    ...
    with tempfile.TemporaryDirectory(prefix="ks-", dir="/tmp") as temp_dir:
        ...
        try:
            try:
                ...
                pytest.skip(f"Unix socket binding is blocked by the test sandbox: {exc}")
            with pytest.raises(OSError, match="不是普通文件"):
...
def test_deploy_isolation_failure_does_not_write_md_or_config(tmp_path, monkeypatch):
    ...
    assert hooks_path.read_text(encoding="utf-8") == "active hooks\n"
    assert not (codex_dir / "your-rules.md").exists()
...
def test_multi_directory_isolation_failure_rolls_back_first_directory(
    ...
    for directory in (first, second):
        ...
        assert not (directory / "your-rules.md").exists()
...
def test_multi_directory_write_failure_rolls_back_all_directories(
    ...
    for directory in (first, second):
        ...
        (directory / "your-rules.md").write_text(
            original_md,
            encoding="utf-8",
        )
        (directory / "hooks.json").write_text(
    ...
    for directory in (first, second):
        ...
        assert (directory / "your-rules.md").read_text(
            encoding="utf-8"
        ) == original_md
        assert (directory / "hooks.json").read_text(encoding="utf-8") == (
            f"{directory.name} hooks\n"
...
def test_deployment_rollback_preserves_concurrent_config_and_md(
    ...
    def create_concurrent_files_then_fail(path, content, *args, **kwargs):
        ...
        if path == second / "config.toml":
            ...
            os.replace(replacement, first / "your-rules.md")
    ...
    assert (first / "your-rules.md").read_text(encoding="utf-8") == (
        "concurrent md\n"
    )
    assert (second / "config.toml").read_text(encoding="utf-8") == (
        'model = "gpt-5.6"\n'
    )
    assert not (second / "your-rules.md").exists()
...
def test_deployment_rollback_keeps_md_when_config_rollback_conflicts(tmp_path):
    ...
    md_path = tmp_path / "your-rules.md"
    ...
    assert any("保留 your-rules.md" in error for error in errors)
...
def test_missing_file_fails_closed_without_a_bundled_prompt():
    with pytest.raises(FileNotFoundError, match="--file"):
        codex_instruct.load_md_content(None)

    assert "Codex Instructions Example" in codex_instruct.BUILTIN_GPT_UNRESTRICTED_MD


def test_neutral_example_is_only_loaded_as_an_explicit_external_file():
    example = codex_instruct.load_md_content(str(EXAMPLE_PATH))

    assert "Codex Instructions Example" in example
    assert example == codex_instruct.BUILTIN_GPT_UNRESTRICTED_MD
...
def test_cli_yes_writes_to_explicit_codex_dir(tmp_path):
    ...
    assert 'model_instructions_file = "./your-rules.md"' in config.read_text(encoding="utf-8")
    assert (codex_dir / "your-rules.md").exists()
...
def test_cli_status_is_read_only_and_reports_active_hooks(tmp_path):
    ...
    config.write_text('model_instructions_file = "./your-rules.md"\n', encoding="utf-8")
...
def test_cli_migrates_referenced_legacy_file_to_timestamped_archive(tmp_path):
    ...
    config.write_text(
        'model_instructions_file = "./historical-legacy-prompt.md"\n',
        encoding="utf-8",
    )
    ...
    archives = list(codex_dir.glob("historical-legacy-prompt.md.bak_*"))
...
def test_cli_preserves_unmanaged_legacy_file(tmp_path):
    ...
    assert not list(codex_dir.glob("historical-legacy-prompt.md.bak_*"))
...
def test_deploy_rolls_back_legacy_archive_on_later_failure(tmp_path, monkeypatch):
    ...
    config = codex_dir / "config.toml"
    legacy = codex_dir / codex_instruct.LEGACY_MD_FILENAME
    original_config = 'model_instructions_file = "./historical-legacy-prompt.md"\n'
...
def test_deploy_final_sweep_detects_earlier_directory_race(tmp_path, monkeypatch):
    ...
    def race_after_last_manifest(state, content):
        ...
        published += 1
        if published == 2:
            (first / "your-rules.md").write_text(
                "concurrent prompt\n",
                encoding="utf-8",
            )
...
def _make_current_takeover_fixture(tmp_path, name):
    ...
    config_bytes = (
        b'model = "gpt-5.6"\n'
        b'model_instructions_file = "./your-rules.md"\n'
    )
    prompt_bytes = (MODULE_PATH.parent / "examples" / "your-rules.md").read_bytes()
...
def _run_cli(*args):
    ...
    if "--file" not in arguments and not non_deploy_modes.intersection(arguments):
        arguments.extend(("--file", str(EXAMPLE_PATH)))
...
def test_legacy_replacement_after_plan_is_not_archived(tmp_path, monkeypatch):
    ...
    config = codex_dir / "config.toml"
    legacy = codex_dir / codex_instruct.LEGACY_MD_FILENAME
    config.write_text('model_instructions_file = "./historical-legacy-prompt.md"\n', encoding="utf-8")
