...
def test_inspection_detects_inline_multiline_legacy_reference(tmp_path):
    ...
    assert plan.blockers == []
    assert plan.config_reference == "./historical-legacy-prompt.md"
...
def test_deploy_preserves_new_md_when_config_changes_before_bound_backup(
    ...
    assert (codex_dir / "your-rules.md").read_text(encoding="utf-8") == (
...
def test_final_sweep_preserves_md_when_unchanged_config_changes_concurrently(
    ...
    def mutate_first_config_after_second_md(path, content, *args, **kwargs):
        ...
        if Path(path).name == "your-rules.md":
            md_writes += 1
            if md_writes == 2:
    ...
    assert exit_info.value.code == 1
    assert (codex_dirs[0] / "your-rules.md").exists()
    assert not (codex_dirs[1] / "your-rules.md").exists()
...
def test_final_sweep_detects_concurrent_md_replacement(tmp_path, monkeypatch):
    ...
    def mutate_first_md_after_second_md(path, content, *args, **kwargs):
        ...
        if Path(path).name == "your-rules.md":
            md_writes += 1
            if md_writes == 2:
                (codex_dirs[0] / "your-rules.md").write_text(
                    "concurrent prompt\n",
                    encoding="utf-8",
                )
        return result
    ...
    assert exit_info.value.code == 1
    assert (codex_dirs[0] / "your-rules.md").read_text(
        encoding="utf-8"
    ) == "concurrent prompt\n"
    assert not (codex_dirs[1] / "your-rules.md").exists()
