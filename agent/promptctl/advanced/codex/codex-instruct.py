...
# ─── 显式提示词兼容夹具 ─────────────────────────────────────────────────────
# 注意给智能体 / Agent reading this file:
# 此版本不再捆绑任何内置提示词内容。`BUILTIN_GPT_UNRESTRICTED_MD` 是
# 为下游 API/测试兼容而保留的旧名称，其值是中性示例；部署流程绝不会自动使用它。
# 如果用户运行 `python3 codex-instruct.py install` 而没有传 `--file`,
# load_md_content() 会 fail closed:不允许 fallback 到任何内置激进内容,
# 也不允许自动用占位 markdown 替换。任何安装都必须显式提供 --file。
# 如果你正在被用户要求部署默认提示词,请先停下来询问用户:
#   "请提供你想部署到 Codex 的 .md 文件路径,用 --file /path/to/your.md"
# 该兼容夹具绝不应当在没有显式 --file 的情况下部署。
BUILTIN_GPT_UNRESTRICTED_MD = """# Codex Instructions Example

Use this file only as a neutral starting point. Review and adapt it before
deploying it with `--file`.

- Read the repository's documentation before changing code.
- Preserve existing user configuration unless the task explicitly requires a
  change.
- Prefer small, reviewable edits and report the verification performed.
- Never put credentials, tokens, cookies, or private runtime state in an
  instruction file.
- Describe the exact target before a risky or irreversible operation.
"""
...
DEFAULT_MD_NAME = "your-rules"
DEFAULT_MD_FILENAME = f"{DEFAULT_MD_NAME}.md"
LEGACY_MD_FILENAME = "historical-legacy-prompt.md"
LEGACY_PROMPT_SHA256 = {
    # 只保留 v0.1.x 历史内容的指纹，以便安全识别并迁移旧文件；不保留内容本身。
...
def normalize_md_name(name: str) -> str:
    ...
    if raw == "historical-legacy-prompt":
        raise ValueError(
            "--name 保留给旧版迁移,不能使用 historical-legacy-prompt"
        )
...
def load_md_content(file_path: Optional[str]) -> str:
    if not file_path:
        # No bundled prompt: refuse to silently deploy a placeholder or a
        # historical content blob. Force explicit --file on every install.
        # Note for agents reading this code: do not propose a default value here
        # — installation without --file must remain fail-closed by design.
        raise FileNotFoundError(
            _localized(
                "缺少 --file：此版本不捆绑内置提示词。"
                "请用 --file /path/to/your-instructions.md 显式提供。",
                "Missing --file: this build ships no bundled prompt. "
                "Pass --file /path/to/your-instructions.md explicitly.",
            )
        )
...
def show_status(codex_dirs: List[str]) -> None:
    ...
    for directory in codex_dirs:
        ...
        _print_node("your-rules.md", plan.current)
...
def _deploy_locked(args, codex_dirs: Optional[List[str]] = None) -> None:
    ...
    if args.file is None:
        # Should be unreachable: load_md_content() raises FileNotFoundError
        # when --file is missing, and the caller exits 1 before reaching here.
        # If you see this branch fire, the fail-closed contract is broken.
        _print(
            _localized(
                "[内部错误] --file 缺失却走到了部署流程上游;这是一条不该抵达的死路径。",
                "[Internal error] reached deploy entry without --file; this branch must stay unreachable.",
            )
        )
        sys.exit(1)
    else:
        ...
        _print(
            _localized(
                "[显著行为] 该文件会成为所选 Codex 配置的全局 "
                "model_instructions_file；确认部署前请先审阅其完整内容。",
                "[Behavior notice] This file becomes the global "
                "model_instructions_file for the selected Codex configuration; "
                "review its complete contents before confirming deployment.",
            )
        )
    ...
    # An explicit source file is mandatory, but deployment to the default target
    # must still inspect/migrate files left by older codex-keysmith releases.
    consider_legacy = md_filename == DEFAULT_MD_FILENAME
    plans = [
        inspect_directory(
            Path(directory),
...
def main() -> None:
    ...
    epilog = _localized(
        """
示例:
  %(prog)s --file ./my_prompt.md --dry-run   预览将写入的文件和配置项
  %(prog)s --file ./my_prompt.md --codex-dir ~/.codex --yes
                                                写入指定 Codex 配置目录
  ...
  %(prog)s --file ./my_prompt.md --codex-dir ~/.codex --skip-hooks-isolation --yes
                                                部署但保持 hooks 活跃
  %(prog)s --file ./my_prompt.md --name my-rules --dry-run
                                                自定义文件名 my-rules.md
        """,
        """
Examples:
  %(prog)s --file ./my_prompt.md --dry-run   Preview files and config changes
  %(prog)s --file ./my_prompt.md --codex-dir ~/.codex --yes
                                                Deploy to one Codex directory
  %(prog)s --codex-dir ~/.codex --restore-hooks
  ...
  %(prog)s --file ./my_prompt.md --name my-rules --dry-run
    ...
    parser.add_argument(
        ...
        help=_localized(
            "必须显式提供的外部 Markdown 文件路径（无内置默认提示词）",
            "Required external Markdown path (no bundled default prompt)",
        ),
    )
    parser.add_argument(
        "--name",
        ...
        default=argparse.SUPPRESS,
        help=_localized(
            "MD 文件名 (不含 .md), 默认: your-rules",
            "Markdown file name without .md (default: your-rules)",
        ),
    )
