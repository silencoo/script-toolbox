...
- The script-toolbox snapshot requires an explicit external Markdown through
  `--file`; it never deploys a bundled prompt implicitly.
- The default destination is now `your-rules.md`, and
  `examples/your-rules.md` is a neutral opt-in starting point.
- The prompt-bank/live-model subsystem tied to the removed historical prompt
  is no longer part of this embedded tool.

### Fixed

- Completed the source working tree's partial explicit-file migration across
  deployment, legacy inspection, localization, documentation, tests, release
  assets, and uninstall/recovery contracts.
- Preserved only historical prompt SHA-256 values so older managed content can
  be recognized and archived without redistributing its body.
