...
提交前删除 token、cookie、用户名、私人路径、完整配置、模型响应和其他可识别数据。维护者会通过 GitHub Security Advisories 跟进；本仓库不承诺固定响应时限。
...
Remove tokens, cookies, usernames, private paths, complete configuration, model responses, and other identifying data. Maintainers will follow up through GitHub Security Advisories; this repository does not promise a fixed response time.
